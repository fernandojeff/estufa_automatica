...
const char* ssid = "LABProv";
const char* password = "labprov1";

// MQTT Connection set 

#define MQTT_ID "ESP32_ESTUFA_TCC_IFRJ_CNIT" //Define o ID do ESP 
#define MQTT_BROKER "broker.hivemq.com" // broker grátis
#define MQTT_PORT 1883
#define MQTT_MILLIS_TOPIC "ifrj_cnit_estufatcc_millis32" //topico millis 
#define MQTT_MILLIS_IPESP "ifrj_cnit_estufatcc_ip_esp32" //topico millis 
...
void loop() {
  ...
  // Build the stream URL with IP address and port
  String streamUrl = "http://" + WiFi.localIP().toString() + ":81/stream";
  const char* streamUrlChar = streamUrl.c_str();
  MQTT.publish(MQTT_MILLIS_IPESP, streamUrlChar); // publish IP address in topic
